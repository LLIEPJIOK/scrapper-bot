...
const (
	migrationDir = "../../../../migrations/scrapper"
)

type ScrapperSuite struct {
	suite.Suite
	pool   *pgxpool.Pool
...
func (s *ScrapperSuite) BeforeAll(t provider.T) {
...
func (s *ScrapperSuite) BeforeEach(t provider.T) {
	ctx := context.Background()
...
func (s *ScrapperSuite) AfterEach(t provider.T) {
	ctx := context.Background()
...
func (s *ScrapperSuite) AfterAll(t provider.T) {
	err := s.db.Close()
...
func TestSuite(t *testing.T) {
	suite.RunSuite(t, new(ScrapperSuite))
}

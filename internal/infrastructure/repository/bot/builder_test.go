...
func (s *BotSuite) TestAddAndGetUpdates_Builder(t provider.T) {
	ctx := context.Background()

	repo := bot.NewBuilder(s.pool)
...
func (s *BotSuite) TestGetUpdatesEmpty_Builder(t provider.T) {
	ctx := context.Background()

	BuilderRepo := bot.NewBuilder(s.pool)

	from := time.Now().Add(-10 * time.Minute)
	to := time.Now().Add(-5 * time.Minute)

	chats, err := BuilderRepo.GetUpdatesChats(ctx, from, to)
	require.NoError(t, err)
	require.Empty(t, chats)

	updates, err := BuilderRepo.GetUpdates(ctx, 1, from, to)
	require.NoError(t, err)
	require.Empty(t, updates)
}

func (s *BotSuite) TestGetUpdatesMultipleChats_Builder(t provider.T) {
	ctx := context.Background()
	...
	for _, upd := range updatesToAdd {
		err := BuilderRepo.AddUpdate(ctx, upd)
		require.NoError(t, err)
	}
...
func (s *BotSuite) TestEdgeCaseTimeFiltering_Builder(t provider.T) {
	ctx := context.Background()
	...
	err := BuilderRepo.AddUpdate(ctx, update)
	require.NoError(t, err)

	updates, err := BuilderRepo.GetUpdates(ctx, 1, now.Add(-1*time.Minute), now.Add(1*time.Minute))
	require.NoError(t, err)
	require.Len(t, updates, 1)
	...
	filtered, err := BuilderRepo.GetUpdates(ctx, 1, from, to)
	require.NoError(t, err)

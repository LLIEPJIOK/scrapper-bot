...
func (s *ScrapperSuite) TestRegisterAndDeleteChat_SQL(t provider.T) {
	ctx := context.Background()

	repo := scrapper.NewSQL(s.pool)
...
func isChatRegistered(ctx context.Context, db *pgxpool.Pool, chatID int64) (bool, error) {
	var exists bool

	query := `SELECT COUNT(*) > 0 FROM chats WHERE id = $1 AND deleted_at IS NULL`

	err := db.QueryRow(ctx, query, chatID).Scan(&exists)

	return exists, err
}

func (s *ScrapperSuite) TestTrackUntrackLink_SQL(t provider.T) {
	ctx := context.Background()
...
func (s *ScrapperSuite) TestGetLink_SQL(t provider.T) {
	ctx := context.Background()
...
func (s *ScrapperSuite) TestListLinks_SQL(t provider.T) {
	ctx := context.Background()
...
func (s *ScrapperSuite) TestGetCheckLinks_SQL(t provider.T) {
	ctx := context.Background()
...
func (s *ScrapperSuite) TestGetCheckLinks_Pagination_SQL(t provider.T) {
	ctx := context.Background()
...
func (s *ScrapperSuite) TestGetActiveLinks_SQL(t provider.T) {
	ctx := context.Background()
	repo := scrapper.NewSQL(s.pool)

...
func TestTrackAddTimeSetterImmediately_Handle_InvalidObject(t *testing.T) {
	t.Parallel()

	channels := domain.NewChannels()
	setter := processor.NewTrackAddTimeSetterImmediately(channels)

	state := &processor.State{
		ChatID:    123,
		MessageID: 456,
		FSMState:  "track_add_set_time_immediately",
...
func TestTrackAddTimeSetterImmediately_Handle_SuccessWithKeyboard(t *testing.T) {
	t.Parallel()
	...
	assert.True(t, updatedLink.SendImmediately.Value, "SendImmediately should be true")
...
func TestTrackAddTimeSetterImmediately_Handle_SuccessWithoutKeyboard(t *testing.T) {
	t.Parallel()
